...
class PlaceController extends Controller
{
    ...
    {
        $places = Place::orderBy('order_number', 'asc')
            ->when($request->filled('placeName'), function ($q) use ($request){
                $q->where('name', 'like', '%'. $request->placeName. '%');
            })
            ->paginate(6);
    ...
    public function store(PlaceAddRequest $request)
    {
        ...
        return to_route('admin.place.index')->with('response', [
           'status' => "success",
    ...
    public function todayReport(Place $place, Request $request)
    {
        ...
        return view('admin.place.report.index', compact('place', 'case'));
    }
