@if (!Session::has('table')){{-- Masa Sipariş Değilse--}}
    <div class="orderType">
        ...
        <div class="orderType_selection @if ($errors->has('orderDetails.orderType')) error @endif">
            ...
            @endif
            @if ($place->package_order)
                <div>
                    <input type="radio" wire:click="handleOrderType('packageOrder')"
                    value="packageOrder" name="orderType_selection" id="orderType_selection2">
                    <label for="orderType_selection2">{{ __('Paket Sipariş') }} </label>
                </div>
            @endif
        </div>
        @if ($errors->has('orderDetails.orderType'))
            <div class="alert alert-danger" role="alert">
                {{ $errors->first('orderDetails.orderType') }}
            </div>
        @endif
        <div class="paymentType_selection  @if ($errors->has('orderDetails.paymentType')) error @endif">
            <div>
                <input type="radio" value="cash" wire:model.defer="orderDetails.paymentType"
                       name="paymentType_selection" id="paymentType_selection1">
                <label for="paymentType_selection1">Nakit </label>
            </div>
            <div>
                <input type="radio" value="creditCard" wire:model.defer="orderDetails.paymentType"
                       name="paymentType_selection" id="paymentType_selection2">
                <label for="paymentType_selection2">Kredi Kartı </label>
            </div>
        </div>
        @if ($errors->has('orderDetails.paymentType'))
            <div class="alert alert-danger" role="alert">
                {{ $errors->first('orderDetails.paymentType') }}
            </div>
        @endif
    </div>
    <div class="otherInfos">
        <form action="">
            <div class="title">{{ __('Diğer Bilgiler') }}</div>
            <div class="formItem">
                <input type="text" class="@if ($errors->has('orderDetails.name')) error @endif"
                       wire:model.defer="orderDetails.name" placeholder="İsim / Soyisim">
                <input type="text" wire:model.defer="orderDetails.phone"
                       class="@if ($errors->has('orderDetails.phone')) error @endif" placeholder="+90">
            </div>
            @if ($errors->has('orderDetails.name') || $errors->has('orderDetails.phone'))
                <div class="alert alert-danger" role="alert">
                    {{ $errors->first('orderDetails.name') }}
                    {{ $errors->first('orderDetails.phone') }}
                </div>
            @endif
            <div class="formItem">
                    <textarea name="" class="@if ($errors->has('orderDetails.address')) error @endif" wire:model.defer="orderDetails.address"
                              placeholder="Adres" id="" cols="30" rows="10"></textarea>
            </div>
            @if ($errors->has('orderDetails.address'))
                <div class="alert alert-danger" role="alert">
                    {{ $errors->first('orderDetails.address') }}
                </div>
            @endif
            <div class="title">{{ __('Sipariş Notu') }}</div>
            <div class="formItem">
                    <textarea class="@if ($errors->has('orderDetails.note')) error @endif" wire:model.defer="orderDetails.note" name=""
                              placeholder="Sipariş Notunuzu Buraya Yazınız" id="" cols="30" rows="10"></textarea>
            </div>
            @if ($errors->has('orderDetails.note'))
                <div class="alert alert-danger" role="alert">
                    {{ $errors->first('orderDetails.note') }}
                </div>
            @endif
        </form>
    </div>
    <div class="bottomsaw">
        <div>
            <button>
                <a style="color:white;"
                   @if (isset($place->phone_number)) href="tel:{{ $place->phone_number }}" @endif>
            ...
            @if (isset($checkIsActiveWhatsAppOrder) && $checkIsActiveWhatsAppOrder->active)
                <button wire:click="{{ isset($this->carts) ? 'saveOrder()' : '' }} ">
...
@else
    ...
    <div class="bottomsaw">
        <div>
            ...
            @if (isset($checkIsActiveWhatsAppOrder) && $checkIsActiveWhatsAppOrder->active)
                <button wire:click="{{ isset($this->carts) ? 'handleOrder()' : '' }} ">
                    <svg xmlns="http://www.w3.org/2000/svg" width="51.175" height="51.391"
                         viewBox="0 0 51.175 51.391">
                        <g id="Group_2578" data-name="Group 2578" transform="translate(-10167.08 -11210.955)">
                            <g id="Group_2576" data-name="Group 2576">
